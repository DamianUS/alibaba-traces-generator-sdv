...
def generate_experiment_command(trace, sample_size, n_samples, seq_len, epochs, experiment_save_dir):
   return f'python train_sdv.py --ori_data_filename ~/DataCenter-Traces-Datasets/azure_v2/vm_cpu_readings_month_aggregated_cpu_mem_no_timestamp.csv --trace {trace} --sample_size {sample_size} --n_samples {n_samples} --seq_len {seq_len} --epochs {epochs} --experiment_save_dir {experiment_save_dir}\n'
...
experiment_save_dir = '~/experiments/sdv/azure_v2/'
trace = 'azure_v2'
...
generate_bash_file(commands,'sdv-azure_v2', trace)